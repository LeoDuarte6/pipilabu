...
These gates turn Leo and Jon's voice session into bounded tests. Stop after each gate, play together, record the result, and only then choose the next build.
...
Only after A–F pass, add one treatment station and one treated order. Validate `apple color × treatment × request` before building potion depth, market competition, random disasters, or progression systems.
